...
from datastore.providers.hologres_datastore import (
    OUTPUT_DIM,
    HologresDataStore,
)
...
@pytest.fixture
def hologres_datastore():
    return HologresDataStore()
...
@pytest.mark.asyncio
async def test_upsert(hologres_datastore, document_chunk_one):
    await hologres_datastore.delete(delete_all=True)
    res = await hologres_datastore._upsert(document_chunk_one)
    ...
    query_results = await hologres_datastore._query(queries=[query])
    assert 3 == len(query_results[0].results)


@pytest.mark.asyncio
async def test_reload(hologres_datastore, document_chunk_one, document_chunk_two):
    await hologres_datastore.delete(delete_all=True)
    ...
    query_results = await hologres_datastore._query(queries=[query])
    assert 3 == len(query_results[0].results)
    new_store = HologresDataStore()
    another_in = {i: document_chunk_two[i]
                  for i in document_chunk_two if i != res[0]}
...
@pytest.mark.asyncio
async def test_upsert_query_all(hologres_datastore, document_chunk_two):
    await hologres_datastore.delete(delete_all=True)
    res = await hologres_datastore._upsert(document_chunk_two)
...
@pytest.mark.asyncio
async def test_query_accuracy(hologres_datastore, document_chunk_one):
    await hologres_datastore.delete(delete_all=True)
    res = await hologres_datastore._upsert(document_chunk_one)
    assert res == list(document_chunk_one.keys())
    query = QueryWithEmbedding(
...
@pytest.mark.asyncio
async def test_query_filter(hologres_datastore, document_chunk_one):
    await hologres_datastore.delete(delete_all=True)
    res = await hologres_datastore._upsert(document_chunk_one)
    assert res == list(document_chunk_one.keys())
    query = QueryWithEmbedding(
...
@pytest.mark.asyncio
async def test_delete_with_date_filter(hologres_datastore, document_chunk_one):
    await hologres_datastore.delete(delete_all=True)
    res = await hologres_datastore._upsert(document_chunk_one)
    assert res == list(document_chunk_one.keys())
    await hologres_datastore.delete(
        filter=DocumentMetadataFilter(
...
@pytest.mark.asyncio
async def test_delete_with_source_filter(hologres_datastore, document_chunk_one):
    await hologres_datastore.delete(delete_all=True)
    res = await hologres_datastore._upsert(document_chunk_one)
    assert res == list(document_chunk_one.keys())
    await hologres_datastore.delete(
...
@pytest.mark.asyncio
async def test_delete_with_document_id_filter(hologres_datastore, document_chunk_one):
    await hologres_datastore.delete(delete_all=True)
    res = await hologres_datastore._upsert(document_chunk_one)
    assert res == list(document_chunk_one.keys())
    await hologres_datastore.delete(
...
@pytest.mark.asyncio
async def test_delete_with_document_id(hologres_datastore, document_chunk_one):
    await hologres_datastore.delete(delete_all=True)
    res = await hologres_datastore._upsert(document_chunk_one)
    assert res == list(document_chunk_one.keys())
    await hologres_datastore.delete([res[0]])
