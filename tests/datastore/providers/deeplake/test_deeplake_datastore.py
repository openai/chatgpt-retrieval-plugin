...
from datastore.providers.deeplake_datastore import (
    DeepLakeDataStore,
)
...
@pytest.mark.asyncio
async def test_upsert(deeplake_datastore, document_chunk_one):
    await deeplake_datastore.delete(delete_all=True)
    res = await deeplake_datastore._upsert(document_chunk_one)
    assert res == list(document_chunk_one.keys())
    deeplake_datastore.ds.flush()
    assert 3 == len(deeplake_datastore.ds)


@pytest.mark.asyncio
async def test_reload(deeplake_datastore, document_chunk_one, document_chunk_two):
    await deeplake_datastore.delete(delete_all=True)
    ...
    deeplake_datastore.ds.flush()
    assert 3 == len(deeplake_datastore.ds)
    new_store = DeepLakeDataStore()
    another_in = {i:document_chunk_two[i] for i in document_chunk_two if i!=res[0]}
    res = await new_store._upsert(another_in)
    new_store.ds.flush()
    assert 6 == len(new_store.ds) # here was deeplake_datastore, basicaly previous pointer doesn't get updated
    query = QueryWithEmbedding(
        query="lorem",
        top_k=10,
        embedding=[0.5] * OUTPUT_DIM,
    )
    query_results = await new_store._query(queries=[query])
    assert 1 == len(query_results)


@pytest.mark.asyncio
async def test_upsert_query_all(deeplake_datastore, document_chunk_two):
    await deeplake_datastore.delete(delete_all=True)
    res = await deeplake_datastore._upsert(document_chunk_two)
    ...
    query_results = await deeplake_datastore._query(queries=[query])
...
@pytest.mark.asyncio
async def test_query_accuracy(deeplake_datastore, document_chunk_one):
    await deeplake_datastore.delete(delete_all=True)
    res = await deeplake_datastore._upsert(document_chunk_one)
    assert res == list(document_chunk_one.keys())
    deeplake_datastore.ds.flush()
    ...
    assert 0 == int(query_results[0].results[0].score)
    assert "abc_123" == query_results[0].results[0].id
    


@pytest.mark.asyncio
async def test_query_filter(deeplake_datastore, document_chunk_one):
    await deeplake_datastore.delete(delete_all=True)
    res = await deeplake_datastore._upsert(document_chunk_one)
    assert res == list(document_chunk_one.keys())
    deeplake_datastore.ds.flush()
...
@pytest.mark.asyncio
async def test_delete_with_date_filter(deeplake_datastore, document_chunk_one):
    await deeplake_datastore.delete(delete_all=True)
    res = await deeplake_datastore._upsert(document_chunk_one)
    assert res == list(document_chunk_one.keys())
    deeplake_datastore.ds.flush()
    await deeplake_datastore.delete(
        filter=DocumentMetadataFilter(
...
@pytest.mark.asyncio
async def test_delete_with_source_filter(deeplake_datastore, document_chunk_one):
    await deeplake_datastore.delete(delete_all=True)
    res = await deeplake_datastore._upsert(document_chunk_one)
    assert res == list(document_chunk_one.keys())
    deeplake_datastore.ds.flush()
...
@pytest.mark.asyncio
async def test_delete_with_document_id_filter(deeplake_datastore, document_chunk_one):
    await deeplake_datastore.delete(delete_all=True)
    res = await deeplake_datastore._upsert(document_chunk_one)
    assert res == list(document_chunk_one.keys())
    deeplake_datastore.ds.flush()
...
@pytest.mark.asyncio
async def test_delete_with_document_id(deeplake_datastore, document_chunk_one):
    await deeplake_datastore.delete(delete_all=True)
    res = await deeplake_datastore._upsert(document_chunk_one)
    assert res == list(document_chunk_one.keys())
    deeplake_datastore.ds.flush()
    await deeplake_datastore.delete([res[0]])
